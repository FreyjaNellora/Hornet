//! Corpus replay: resolve PGN4 ply tokens against the move generator, with the fidelity
//! inferences from EXP-028. **The one shared implementation** — the replay test, the tuner, and
//! every instrument example use these, so a fidelity fix lands everywhere at once.
//!
...
//! - **DKW inference (EXP-028):** a live king can never capture its own piece, so a corpus king
