...
pub fn generate_pseudo_legal(board: &Board) -> Vec<Move> {
    ...
    if board.is_dkw(mover) {
        ...
        // (a dead king may take its own frozen piece); under rule 0 frozen walls block it.
...
fn on_promotion_edge(player: Player, sq: Square) -> bool {
    ...
    // wrong (see CO-003); confirmed empirically.
    match player {
...
mod tests {
    ...
    fn perft_matches_known_values() {
        // Perft against the known 4PC reference counts (clean-room rebuild, zero shared code).
        let mut b = start();
    ...
    fn dkw_king_walks_and_may_take_even_its_own_pieces() {
        // EXP-026 rule 2: the walking king may capture any adjacent piece,
        // including its OWN frozen army (a dead king may take its own frozen piece).
        use crate::board::types::{Piece, PieceType};
        let at = |s: &str| Square::from_algebraic(s).unwrap();
        let mut b = Board::empty();
        b.side_to_move = Player::Red;
        ...
        assert!(
            ...
            "the DKW king MAY capture its own frozen pawn (rule 2)"
        );
        assert!(
