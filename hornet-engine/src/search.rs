...
pub struct Searcher {
    ...
    /// Kimi's elimination-proximity (fires late on a collapsing opponent — a *finishing* gradient).
    ...
    danger_weight: i16,
    /// King-danger SHAPE: `false` = linear scalar (defender-mitigated); `true` = Kimi's non-linear
...
impl Searcher {
    ...
    /// Use Kimi's elimination-proximity win signal (finishing gradient) instead of banked FFA points
    ...
    /// Use the non-linear attack-units danger **table** (Kimi's shape) instead of the linear scalar.
    ...
    /// (`win_weight ×` mean-relative FFA points, the *goal*) and the king-danger term (`danger_weight ×`
    /// the points-risk of elimination). With both weights `0` this is exactly the static eval (the
    ...
    fn eval_with_win(&mut self, board: &Board) -> [i16; 4] {
        ...
        if self.win_weight != 0 {
            if self.win_proxy {
                // Kimi's elimination-proximity: win_i = Σ_{j≠i} prox_j − 3·prox_i = total − 4·prox_i.
    ...
    pub fn search_flashlight(
        ...
        // One node per kept tree position: parent index, the mover *at* this node, the move that
        // reached it (None for the root), the backed-up value, and whether it's a leaf.
        struct FNode {
            parent: usize,
            mover: usize,
            mv: Option<Move>,
        ...
        // keeps the child that maximizes the parent's own component.
        ...
        // Best root move = the root child maximizing the root player's component.
